				...
				<form @submit.prevent="form.post(route('magazines.store'))">
					<div class="space-y-4">
						<div class="space-y-1">
							...
							<input class="w-full px-4 py-2 border bg-gray-200 border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500" type="text" name="name" v-model="form.name">
						</div>
						<div class="space-y-1">
							<label for="image_url" class="text-md font-medium text-gray-700">
								Image
							</label>
							<input class="bg-gray-200 w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-indigo-500 focus:border-indigo-500" type="file" name="image_url" @change="previewImage" @input="form.image_url = $event.target.files[0]">
